...
export type UserGetByPageResult = Result<[entities: UserCreatedResponse[], total: number], DatabaseException>
...
export class UserInfrastructure implements UserRepository{
  ...
  async save(user: User): Promise<UserResult>{
   ...
   async getById(id: string): Promise<UserDomainResult>{
    try{
      const userRepository = MySQLBootstrap.dataSource.getRepository(UserEntity);
      const userEntity = await userRepository.findOne({where: {id, isActive: true}, relations:["roles"]});
  ...
  async getByEmail(email: string): Promise<UserDomainResult>{
    try{
      const userRepository = MySQLBootstrap.dataSource.getRepository(UserEntity);
      const userEntity = await userRepository.findOne({where: {email, isActive: true}, relations:["roles"]});
  ...
  async getByPage(page: number, pageSize: number): Promise<UserGetByPageResult>{
    try{
      const UserRepository = MySQLBootstrap.dataSource.getRepository(UserEntity);
      ...
      const entities = UserModelDto.fromDataToResponse(userEntities) as UserCreatedResponse[];
